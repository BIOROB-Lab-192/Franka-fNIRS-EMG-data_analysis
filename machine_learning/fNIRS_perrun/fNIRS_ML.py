...
1. Honest leave-one-run-out cross-validation (LOOCV)
...
2. Final production training
   ...
   - Save the final all-runs model, normalization stats, and config.
...
# Recommended safer training settings for the original CNN architecture.
# These are more conservative than the original 100-epoch, lower-regularization setup.
LR = 5e-4
WEIGHT_DECAY = 5e-3
NUM_EPOCHS = 50
CLASSIFIER_DROP = 0.5
...
# If you want to reproduce your earlier corrected baseline instead, use:
# LR = 1e-3
# WEIGHT_DECAY = 1e-3
# NUM_EPOCHS = 100
# CLASSIFIER_DROP = 0.3
...
def extract_windows_from_run(run_df):
...
def build_fold_data(runs, held_out_run):
    ...
    for run_id, info in runs.items():
        run_windows, run_instances = extract_windows_from_run(info["df"])
...
def main():
    ...
    final_train_ds, final_mean, final_std = build_all_run_data(runs)
